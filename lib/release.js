...
async function commitFiles(distJson) {
  commands.log('Committing files...');
  await exec('git add --all');
  await exec(`git commit -am "Release ${distJson.name}-${distJson.version}"`);
}
...
async function createTag(distJson) {
...
exports.run = async (versionArg) => {
  try {
    ...
    await commitFiles(distJson);
    await pushFiles();
    await createTag(distJson);
    await pushTag();
