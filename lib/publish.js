...
async function buildDockerImage(packageJson) {
...
async function publishToECR(packageJson) {
  commands.log('Signing in to Amazon ECR...');
  let ecrLogin = 'aws ecr get-login --no-include-email';
...
exports.run = async () => {
  try {
    ...
    if (dockerProject) {
      await buildDockerImage(packageJson);
    }
    ...
    if (dockerProject && dockerProject.ecr) {
      await publishToECR(packageJson);
    }
    if (npmProject) {
      await publishToNpm();
    }
